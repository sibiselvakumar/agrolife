...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	$albumsize=mysql_real_escape_string($_POST['albumsize']);
	...
	$status="Order Not Process Yet";
		$uid = $_SESSION['id'];
			 ...
			 $result = mysql_query("SELECT max(sno)as max FROM album");
									 ...
									 $id=$row['max']+1;
											...
											{								
												$filename=$id;				
												$allowedExts = array("jpg","png","bmp","gif","jpeg","pdf","zip","rar");
												$temp = explode(".", $_FILES["file"]["name"]);
												$extension = end($temp);
												if (in_array($extension, $allowedExts))
												{
												move_uploaded_file($_FILES["file"]["tmp_name"],"album/$filename.$extension" );
												}
												$path="album/$filename.$extension";		 
											}
									
	date_default_timezone_set('Asia/Kolkata'); 
$currentdate= date("Y-m-d H:i:s"); // time in India								 

mysql_query ("insert into album(albumsize,qty,price,total,sno,path,uid,orderid,cdate,status) values ('$albumsize','$qty','$price','$total','$id','$path','$uid','$orderid','$currentdate','$status')");
...
function submit1(){
            ...
            var result = first_number * second_number + 250;
			...
			<div class="row">		
				<div class="col-md-12 col-sm-12 already-registered-login">
					...
					<form class="register-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
					  ...
					  <div class="form-group">
					    <label class="info-title">Album Size<span>*</span></label>
					    <select class="form-control unicase-form-control text-input" name="albumsize" required>
                        	<option value=""> Choose Album Size </option>
                      ...
                      <div class="form-group">
					    <label class="info-title" for="Billing State ">Photos Quantity<span>*</span></label>
			 			<input type="text" class="form-control unicase-form-control text-input" required name="qty" id="qty">
					  </div>
                      
                      <div class="form-group">
					    <label class="info-title" for="Billing State ">Price Per Photo <span>*</span></label>
			 			<input type="text" class="form-control unicase-form-control text-input" required name="price" id="price" value="10" readonly>
					  </div>
                      
                      <div class="form-group">
					    <label class="info-title" for="Billing State ">Album Price <span>*</span></label>
			 			<input type="text" class="form-control unicase-form-control text-input" value="250" readonly>
					  </div>
                      ...
                      <div class="form-group">
						...
						<div class="form-group">
					    <label class="info-title" for="Billing State ">Photos <span>*</span></label>
